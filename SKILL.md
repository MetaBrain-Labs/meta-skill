...
```Markdown
1. 与用户深度交流，了解用户，理解用户需求，判断用户已经处在哪个阶段。
2. 根据用户需求，明确 skill 的功能和范围。
3. 通过可用的工具、向用户询问等方式了解当前 AI Agent 的使用环境。
4. 搜索相关的 skill，评估适用性和质量，向用户推荐最合适的几个 skill 供用户选择，并从用户需求、适用性和质量角度出发向用户解释每个 skill 的推荐理由。
5. 根据用户对搜索结果的满意程度，决定是否改造或创建新的 skill。
6. 改造或创建新的 skill 后，为保障 skill 健壮性和质量，必须完成下列步骤：
   - 审查 skill ，修复出现的错误，提升健壮性。
   - 编写足量测试用例，提高 skill 的质量。
7. 与用户深度交流，根据用户对 skill 的测试结果，调整 skill 。
8. 重复直到用户满意为止。
9. 将 skill 打包为后缀为 `.skill` 的文件交付给用户。
```
